...
- **four.meme 自动候审** — 通过 Bitquery API 实时拉取 BSC 链上最新 Meme 币，自动填入候审队列，每 5 分钟刷新，真实链上数据驱动
- **开庭广场** — 项目方支付 BNB 开庭费，AI 自动分析，社区参与裁决；已结案轻量弹窗展示裁定结果 + 链上领取奖励
- **$GAVEL 代币** — BEP-20，持有量决定陪审员等级和权益
- **Meme 雷达** — DexScreener 真实价格/交易量行情（热度榜/涨幅榜/新币榜），BSC 实时行情可视化
- **🥊 打脸记录** — AI 法官历史裁错案例（LUNA 归零、SQUID 跑路、PEPE 涨 7000%），一键截图生成可分享的打脸图片
- **📱 移动端适配** — 独立移动端 UI（≤768px 自动切换），三页架构：AI 法官聊天（ChatGPT 风格）/ 开庭广场（GMGN 战壕风格）/ 我的
...
| 层 | 技术 |
| --- | --- |
| 前端 | 原生 HTML/CSS/JS，零依赖单文件，移动端独立布局 |
| AI 引擎 | DeepSeek Chat API（链上包公主力引擎） |
| 智能合约 | Solidity 0.8.19，BSC Testnet |
| 链上数据 | Bitquery GraphQL API + DexScreener API + Etherscan V2 API |
...
1. four.meme 新币自动进入候审队列
   └── Bitquery API 每 5 分钟拉取最新交易数据
   └── 按 BNB 交易量排序，取前 6 个
   └── DexScreener 补充真实价格 / 涨跌幅 / 流动性
...
| Tab | 功能 | 设计参考 |
| --- | --- | --- |
| ⚖️ 法官 | AI 法官聊天问答，气泡对话界面 | ChatGPT / Claude |
| 🏛️ 开庭 | 案件列表（候审/进行中/已结案）+ 全屏详情页押注 | GMGN 战壕 |
| 👤 我的 | 钱包余额 + 押注记录 | GMGN 资产 |
...
- **一键截图**：html2canvas 生成打脸卡片 PNG，直接发推 / 发群
...
- ✅ four.meme 新币自动候审（Bitquery 实时数据）
- ✅ Meme 雷达真实行情（DexScreener 价格/交易量）
- ✅ 裁定哈希链上存证（手动触发）
- ✅ claimNewbie 新人礼包
- ✅ createCase 链上开庭
- ✅ 已结案弹窗 + claimGavel 领取奖励
- ✅ $GAVEL 陪审员等级体系
- ✅ 打脸记录 + 截图分享
- ✅ 移动端三页 UI（自动适配）
